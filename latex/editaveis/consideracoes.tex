...
\newpage
Após o período de inicialização, colocando o sistema em modo de alta performance (NM\_MODE) é possível ver que as tasks se estabilizaram e começaram a diminuir o processamento do kernel. O processamento medio ficou em torno de 8\%. Nesse estado o \textit{TaskManager} é que consome mais que os outras \textit{Tasks}, em torno de 2\%. Tambem é possivel notar que . Figura \ref{trace_snapshoot_3}
...
\newpage
Colocando o sistema em modo de baixo consumo é possível notar que há uma redução no tempo de execução das \textit{Tasks}, que durante os testes esse tempo foi 5 vezes o tempo de execução nominal, como mostrado na Figura \ref{trace_snapshoot_5}. Também é visto que as \textit{Tasks} com menos prioridade foram coladas em modo de sleep, restando apenas as \textit{Tasks} que são de controle e coleta/armazenamento de dados.
...
Como era de se esperar, durante o modo de hibernação apenas o \textit{TaskManager} fica ativo e consumindo mais CPU do que o usual, quase de 11\%. O kernel tambem executa essa task com um espassamento maior em 5 vezes ao normal. A\ref{trace_snapshoot_4} o OBC sendo executado em modo HB\_MODE.
...
\newpage
Por fim, alternou-se os modos de operação para e o resultado saiu como esperado. Na figura \ref{trace_snapshoot_8}  há 
\begin{figure}[h]
	\centering
...
Com essa sessão foi possível concluir que os estados estão sendo alternados conforme o nível de bateria, que no caso foi simulado pelo nível de intensidade de luz. Com isso o requisitos OBC-SW-R7 foi comprido.
